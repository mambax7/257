...
/**
 * Class ObjectVersionAlreadyExistsException
 * @package XoopsModules\Publisher\versioncontrol
 */
...
/**
 * Class ObjectIdentifierNotFoundException
 * @package XoopsModules\Publisher\versioncontrol
 */
...
abstract class VersionControl
{
     ...
     * @access public
     */
    abstract public function getLatestVersionId($_sIdentifier);
     ...
     * @access public
     */
    abstract public function getLatestVersion_no($_sIdentifier);
     ...
     * @access public
     */
    abstract public function getLatestVersion($_sIdentifier);
     ...
     * @access public
     */
    abstract public function getVersion($_sIdentifier, $_iVersionNo);
     ...
     * @access public
     */
    abstract public function getAllVersion($_sIdentifier);
     ...
     * @access public
     */
    abstract public function objectHashExists($_sIdentifier, $_sHash);
     ...
     */
    private function getNextVersionNumber(&$_sIdentifier)
    {
        return $this->getLatestVersion_no($_sIdentifier) + 1;
    }
     ...
     * @access public
     */
    final public function submitVersion($_sIdentifier, $_aData, $_iEditor, $_sInformations = '')
    {
        $iLatestId = $this->getLatestVersionId($_sIdentifier);

        if ($iLatestId) {
            $sResultData = $this->encode(serialize($_aData));
            $sResultHash = md5($sResultData);
            if ($this->objectHashChanged($_sIdentifier, $sResultHash) && !$this->objectHashExists($_sIdentifier, $sResultHash)) {
                $iDate          = time();
                $iNextVersionNo = $this->getNextVersionNumber($_sIdentifier);
                return $this->write($_sIdentifier, $iDate, $iNextVersionNo, $sResultData, $sResultHash, $_iEditor, $_sInformations);
            } else {
                throw new Exception('Object Version Already Exists');
            }
        } else {
            return $this->initVersion($_sIdentifier, $_aData, $_iEditor, $_sInformations);
        }
    }
     ...
     * @access private
     */
    final private function initVersion($_sIdentifier, $_aData, $_iEditor, $_sInformations = '')
    {
        $sResultData    = $this->encode(serialize($_aData));
        $sResultHash    = md5($sResultData);
        $iDate          = time();
     ...
     * @access private
     */
    final private function objectHashChanged($_sIdentifier, $_sHash)
    {
        $aLatestVersion = $this->getLatestVersion($_sIdentifier);
        if ($aLatestVersion) {
            ...
            throw new Exception('Object Identifier Not Found');
        }
    }
}
