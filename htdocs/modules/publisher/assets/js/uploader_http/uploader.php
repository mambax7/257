    ...
    if (isset($_POST['processed']) && !empty($_POST['folder'])) {
        ...
        if (!$error) {
            ...
            if ($album->exists) {
                ...
                zp_error(gettext('The album could not be created in the “albums” folder. This is usually a permissions problem. Try setting the permissions on the “albums” and “cache” folders to be world-writable using a shell:')
                         . ' <code>chmod 777 '
                         . $AlbumDirName
                         . '/'
                         . CACHEFOLDER
...
if (!isset($_POST['processed'])) {
...
} elseif (!$filecount && !isset($_POST['newalbum'])) {
    ...
    switch ($error) {
        ...
        default:
            $errormsg = sprintf(
                gettext('The error %s was reported when submitting the form. Please try again. If this keeps happening, check your server and PHP configuration (make sure file uploads are enabled, and upload_max_filesize is set high enough.) If you think this is a bug, file a bug report. Thanks!'),
                                $error
            );
            break;
    }
