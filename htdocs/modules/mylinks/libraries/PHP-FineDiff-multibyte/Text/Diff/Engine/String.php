...
class Horde_Text_Diff_Engine_String
{
    ...
    public function diff($diff, $mode = 'autodetect')
    {
        // Detect line breaks.
        $lnbr = "\n";
        if (false !== strpos($diff, "\r\n")) {
            $lnbr = "\r\n";
        } elseif (false !== strpos($diff, "\r")) {
            $lnbr = "\r";
        }
        ...
        if ('autodetect' != $mode && 'context' != $mode && 'unified' != $mode) {
        ...
        if ('autodetect' == $mode) {
            ...
            } elseif (false === $context || false === $unified) {
                $mode = false !== $context ? 'context' : 'unified';
            } else {
        ...
        if (('context' == $mode && 0 === strpos($diff[0], '***'))
            || ('unified' == $mode && 0 === strpos($diff[0], '---'))) {
            array_shift($diff);
            array_shift($diff);
        }
