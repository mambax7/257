...
<div>
    ...
    if (isset($_GET['data'])) {
        if (ctype_alnum($_GET['data'])) {
            ...
            if (false !== $compressed_serialized_data) {
        ...
        $exec_time         = gettimeofday(true) - $start_time;
        if ($diff_opcodes_len) {
            $data_key = sha1(serialize(['granularity' => $granularity, 'from_text' => $from_text, 'diff_opcodes' => $diff_opcodes]));
    ...
    $rendered_diff = FineDiffHTML::renderDiffToHTMLFromOpcodes($from_text, $diff_opcodes, null, (4 != $granularity));
    ...
    ?>
    <div class="panecontainer" style="width:99%"><p>Diff <span style="color:gray">(diff: <?php printf('%.3f', $exec_time); ?> seconds, diff len: <?php echo $diff_opcodes_len; ?> chars)</span>&emsp;/&emsp;Show <input type="radio" name="htmldiffshow" onclick="setHTMLDiffVisibility('deletions');">Deletions
            only&ensp;<input type="radio" name="htmldiffshow" checked="checked" onclick="setHTMLDiffVisibility();">All&ensp;<input type="radio" name="htmldiffshow" onclick="setHTMLDiffVisibility('insertions');">Insertions only</p>
        <div>
    ...
    <form action="viewdiff.php" method="post">
        ...
        <div class="panecontainer" style="display:inline-block;width:49.5%"><p>From</p>
            <div><textarea name="from" class="pane"><?php echo $from_text; ?></textarea></div>
        </div>
        <div class="panecontainer" style="display:inline-block;width:49.5%"><p>To</p>
            <div><textarea name="to" class="pane"><?php echo $to_text; ?></textarea></div>
        </div>
        <p id="params">Granularity:<input name="granularity" type="radio" value="0"<?php if (0 === $granularity) {
                    ...
                    echo ' checked="checked"';
                } ?>>&thinsp;Binary&emsp;<input type="submit" value="View diff">&emsp;<a href="viewdiff.php">
