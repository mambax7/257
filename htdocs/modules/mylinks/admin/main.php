...
function listNewLinks()
{
    ...
    if ($numrows > 0) {
        while (false !== (list($lid, $cid, $title, $url, $logourl, $submitterid) = $xoopsDB->fetchRow($result))) {
...
function modLink()
{
    ...
    $linkimg_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH . '/modules/' . $xoopsModule->getVar('dirname') . '/assets/images/shots/');
    $lid           = Mylinks\Utility::mylinks_cleanVars($_GET, 'lid', 0, 'int', ['min' => 0]);
    $bknrptid      = Mylinks\Utility::mylinks_cleanVars($_GET, 'bknrptid', 0, 'int', ['min' => 0]);
    ...
    while (false !== (list($ratingid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp) = $xoopsDB->fetchRow($result5))) {
    ...
    $x           = 0;
    $colorswitch = '#DDDDDD';
    while (false !== (list($ratingid, $rating, $ratinghostname, $ratingtimestamp) = $xoopsDB->fetchRow($result5))) {
...
{
    global $xoopsDB;
    $lid = Mylinks\Utility::mylinks_cleanVars($_POST, 'lid', 0, 'int', ['min' => 0]);
    $rid = Mylinks\Utility::mylinks_cleanVars($_POST, 'rid', 0, 'int', ['min' => 0]);
...
function listBrokenLinks()
{
    ...
    if (0 == $totalBrokenLinks) {
        ...
        while (false !== (list($reportid, $lid, $sender, $ip) = $xoopsDB->fetchRow($result))) {
...
function ignoreBrokenLinks()
{
    ...
    $bknrptid = Mylinks\Utility::mylinks_cleanVars($_POST, 'bknrptid', 0, 'int', ['min' => 0]);
...
function listModReq()
{
    ...
    if ($totalModRequests > 0) {
        ...
        $lookup_lid = [];
        while (false !== (list($requestid, $lid, $cid, $title, $url, $logourl, $description, $submitterid) = $xoopsDB->fetchRow($result))) {
            ...
            */
            ...
            if ($xoopsModuleConfig['useshots'] && !empty($origlogourl)) {
                echo "<img src='" . XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . "/assets/images/shots/{$origlogourl}' style='width: " . $xoopsModuleConfig['shotwidth'] . ";'>";
            } else {
                echo '&nbsp;';
            }
            echo "</td></tr>\n"
            ...
            if (1 == $xoopsModuleConfig['useshots'] && !empty($logourl)) {
                echo "<img src='" . XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . "/assets/images/shots/{$logourl}' style='width: " . $xoopsModuleConfig['shotwidth'] . ";' alt=''>";
            } else {
                echo '&nbsp;';
            }
...
function changeModReq()
{
    ...
    $requestid = Mylinks\Utility::mylinks_cleanVars($_POST, 'requestid', 0, 'int', ['min' => 0]);
    $query     = 'SELECT lid, cid, title, url, logourl, description FROM ' . $xoopsDB->prefix('mylinks_mod') . " WHERE requestid='{$requestid}'";
    $result    = $xoopsDB->query($query);
    while (false !== (list($lid, $cid, $title, $url, $logourl, $description) = $xoopsDB->fetchRow($result))) {
        $url         = addslashes($url);
...
function modLinkS()
{
    ...
    $cid         = Mylinks\Utility::mylinks_cleanVars($_POST, 'cid', 0, 'int', ['min' => 0]);
    $lid         = Mylinks\Utility::mylinks_cleanVars($_POST, 'lid', 0, 'int', ['min' => 0]);
    $bknrptid    = Mylinks\Utility::mylinks_cleanVars($_POST, 'bknrptid', 0, 'int', ['min' => 0]);
    $url         = Mylinks\Utility::mylinks_cleanVars($_POST, 'url', '', 'string');
    $logourl     = Mylinks\Utility::mylinks_cleanVars($_POST, 'logourl', '', 'string');
    $title       = Mylinks\Utility::mylinks_cleanVars($_POST, 'title', '', 'string');
    $description = Mylinks\Utility::mylinks_cleanVars($_POST, 'description', '', 'string');
    /*
...
function modCat()
{
    ...
    $cid = Mylinks\Utility::mylinks_cleanVars($_GET, 'cid', 0, 'int', ['min' => 0]);
    xoops_cp_header();
...
function modCatS()
{
    global $xoopsDB, $myts, $xoopsModule;
    $cid    = Mylinks\Utility::mylinks_cleanVars($_POST, 'cid', 0, 'int', ['min' => 0]);
    $imgurl = Mylinks\Utility::mylinks_cleanVars($_POST, 'imgurl', '', 'string');
    $title  = Mylinks\Utility::mylinks_cleanVars($_POST, 'title', '', 'string');
...
function delCat()
{
    ...
    $cid = Mylinks\Utility::mylinks_cleanVars($_REQUEST, 'cid', 0, 'int', ['min' => 0]);
    $ok  = Mylinks\Utility::mylinks_cleanVars($_POST, 'ok', 0, 'int', ['min' => 0, 'max' => 1]);
...
{
    global $xoopsDB, $myts, $xoopsModule;
    $pid    = Mylinks\Utility::mylinks_cleanVars($_POST, 'pid', 0, 'int', ['min' => 0]);
    $title  = Mylinks\Utility::mylinks_cleanVars($_POST, 'title', '', 'string');
    $imgurl = Mylinks\Utility::mylinks_cleanVars($_POST, 'imgurl', '', 'string');
    /*
        $title  = $myts->addSlashes($title);
...
function addLink()
{
    ...
    $logourl     = Mylinks\Utility::mylinks_cleanVars($_POST, 'logourl', '', 'string');
    $title       = Mylinks\Utility::mylinks_cleanVars($_POST, 'title', '', 'string');
    $description = Mylinks\Utility::mylinks_cleanVars($_POST, 'descarea', '', 'string');
    /*
        $url           = $myts->addSlashes($url);
...
$op = Mylinks\Utility::mylinks_cleanVars($_REQUEST, 'op', 'main', 'string');
